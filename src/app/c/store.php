...
//    $pcurrent = isset( $_POST['p'] )? $_POST['p']:0;
 ...
 function category(){
 	...
 	$param['name'] = $name =  addslashes(isset($_POST['category'])?$_POST['category']:'');
 	if($name){
 		if($where) {
 	...
 	$param['category_level'] = $category_level =  addslashes(isset($_POST['category_level'])?$_POST['category_label']:'');
 	if($category){
 		if($where) {
 			$where.="  and level = $category_level";
 		}else{
 			$where = " level =  $category_level";
 		}
 		$str.= '&category_level='.$category_level;
 	}
 	/*
 	...
 	*/
 	...
 	$param['records'] = $this->m->get($where.' order by categoryCd asc' , $pcurrent ,  $psize);
