...
<form method="POST"action="<?=BASE?>login/" class="padding" >
    <ul class="panel" >
   <li >
	 <input class="medium" required name="email" placeholder="email" type="email"  value="<?php echo isset($val['email'])?$val['email']:'';?>" />
   </li>
  <li>
      <input class="medium" required placeholder="密码" name="password" type="password" value="<?php echo isset($val['password'])?$val['password']:'';?>" />
